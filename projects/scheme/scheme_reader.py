...
class Pair(object):
    ...
    first and rest. rest must be a Pair or nil
    ...
    def __init__(self, first, rest):
        ...
        if not (rest is nil or isinstance(rest, Pair) or
                type(rest).__name__ == 'Promise'):
            print(rest, type(rest).__name__)
            raise SchemeError(
                f"cdr can only be a pair, nil, or a promise but was {rest}")
        self.first = first
        self.rest = rest
    ...
    def flatmap(self, fn):
        """
        Return a Scheme list after flatmapping
        Python function FN to SELF.
        """
