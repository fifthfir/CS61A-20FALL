...
def num_splits(s, d):
    ...
    def helper(a, b, pre):
        if a == []:
            if abs(sum(a) + pre - sum(b)) <= d:
                return 1
            else:
                return 0
        return helper(a[1:], b + [a[0]], pre) + helper(a[1:], b, pre + a[0])
    return helper(s, [], 0) // 2
...
def align_skeleton(skeleton, code):
    """
    Aligns the given skeleton with the given code, minimizing the edit
    distance between the two. Both skeleton and code are assumed to
    be valid one-line strings of code.
